...
async def _ask_csat_agent(user_id: str, sender: str, text: str, name: str) -> str | None:
    ...
    Returns:
        - String reply kalau CS Agent return reply eksplisit
        - "__cs_handled__" kalau CS Agent sudah handle sendiri (status=ok, dia kirim WA langsung)
        - None kalau gagal total / perlu fallback dari caller
    ...
    Otak / memory:
        Sebelum panggil CS webhook, cek otak_memories. Kalau ada jawaban
        tersimpan (normalized match), langsung pakai — hemat LLM call.
    """
    ...
    try:
        ...
        # === CS Agent return non-ok → generate fallback by intent ===
    ...
    try:
        ...
        if not user_id and text:
            ...
            if csat_reply == "__cs_handled__":
                ...
                await send_wa_reply(phone, reply, inboxid=inboxid, device=device)
                _persist_wa_log(phone, text, reply, user_id=csat_tenant)
                ...
                await send_wa_reply(phone, reply, inboxid=inboxid, device=device)
                _persist_wa_log(phone, text, reply, user_id=csat_tenant)
                return {"status": "ok", "mode": "cs_fallback", "wa_logged": True}
