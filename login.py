        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Masuk", use_container_width=True):
                try:
                    ...
                    st.write("DEBUG res.user", getattr(res, "user", None))
                    st.write("DEBUG res.session", getattr(res, "session", None))
            ...
            if st.form_submit_button("Daftar", use_container_width=True):
                if len(new_password) < 6:
                    ...
                    try:
                        ...
                        if res.user:
                            st.success("✅ Akun berhasil dibuat! Cek email untuk verifikasi, lalu masuk.")
                        else:
                            st.error("Gagal mendaftar.")
                    except Exception as e:
                        st.error(f"Gagal mendaftar: {str(e)[:100]}")
    return False
